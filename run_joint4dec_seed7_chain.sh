#!/bin/bash
# Pod jdec-seed7 (2026-09-12): seed replicate of the joint decoder base (no trajectory head). Stage SF3D + the
...
ulimit -n 65536
E=20260912_joint4_decoder_seed7; CFG=config/joint4_decoder_seed7.yaml
...
sed 's#/workspace/datasets/\(hoi4d_processed_2d_v2\|epic_processed_2d\|arctic_processed_2d\|sf3d_processed_v3\|sf3d_frames_512.lmdb\)#/dev/shm/\1#g' $CFG > /dev/shm/joint4dec_seed7_local.yaml
echo "staged paths: $(grep -c '/dev/shm/' /dev/shm/joint4dec_seed7_local.yaml)"; df -h /dev/shm | tail -1
mkdir -p experiments/$E/logs experiments/$E/checkpoints
echo "=== START $E $(date)"
HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_multi_better.py fit --config /dev/shm/joint4dec_seed7_local.yaml > experiments/$E/logs/train.log 2>&1
