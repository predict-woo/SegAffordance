...
# font sizes (px)
F_BOX, F_SUB, F_ANN, F_MATH, F_TITLE, F_SMALL = 28, 21, 22, 28, 28, 23
FS = 1.45                                   # global font scale (2026-09-16: labels ~8 pt at text width)
...
def text(x, y, s, fs=F_ANN, anchor="middle", fill=INK, math=False, weight="normal", italic=False):
    ...
    add(f'<text x="{x}" y="{y}" font-family="{fam}" font-size="{fs:.1f}" fill="{fill}" text-anchor="{anchor}" '
...
def group(x, y, w, h, title):
    ...
    text(x + w / 2, y + 46, title, F_TITLE, weight="bold", fill="#444")
...
VX, VW = G1X + PAD, 200                   # DINOv3 tower / dino.txt tower
...
AGX, AGW = CBX + CBW + 30, 54             # 1x1 aggregation conv (rotated)
CCX, CCW = AGX + AGW + 30, 54             # CoordConv (rotated)
CH2 = 210                                 # height of the two rotated fusion convs
FW = CCX + CCW - FX                       # width of the exploded FPN
FMX, FMS = FX + FW + 50, 90               # fused map F
TDX, TDW = FMX + FMS + 34, 124             # transformer decoder
G1W = TDX + TDW + PAD - G1X
G2X = G1X + G1W + GGAP                    # readout group
QX, QS = G2X + 66, 120                    # decoded map F_q
BX, BW = QX + QS + 54, 286                # head blocks
HX, HS, HG = BX + BW + 36, 72, 104         # grids (mask, point heatmap)
UX = HX + 2 * HS + HG                     # end of the point map
FLX, VS = BX + BW + 36, 126                # vote-field grid
FEND = FLX + VS
PMX, PMW = FEND + 34, 172                 # part-weighted mean
G2W = max(PMX + PMW, UX + 200) + PAD - G2X
G3X = G2X + G2W + GGAP                    # decoder group
BUS = G3X + 52                            # vertical bus for the fan-in
ADX, ADW, ADH = BUS + 60, 226, 284        # analytic decoder
ADY = MID - ADH / 2
...
for k, ln in enumerate(("“Open the left", "door of the", "closet near", "the window”")):
    text(TX + TW / 2, TROW + (k - 1.5) * 40, ln, F_ANN - 2, italic=True)
text(TX + TW / 2, TY + TH + 36, f'instruction {v("T")}', F_ANN + 1)

# ================================================================ frozen towers
box(VX, VY, VW, VH, "DINOv3\nViT-L/16", FROZEN, sub="frozen")
snowflake(VX + VW - 28, VY + 28, r=13)
arrow([(IX + IS, MID), (VX, MID)])

DH = 196
DY = TROW - DH / 2
box(VX, DY, VW, DH, "dino.txt\ntext encoder", FROZEN, sub="frozen", fs=22)
snowflake(VX + VW - 28, DY + 28, r=13)
...
LV = [(PX + PW, MID - 152 - 33, 66, "/8"), (PX + PW, MID - 22, 44, "/16"), (PX + PW, MID + 152 - 12, 24, "/32")]   # rows at MID -96 / 0 / +96
...
SGX0, SGX1 = FX - 24, CCX + CCW + 24
SGY0, SGY1 = ROWS[0] - CH / 2 - 80, ROWS[2] + CH / 2 + 36
...
text(GX_ + 60, SGY1 + 34, "sentence state", F_SMALL, anchor="start", fill=MUTED)

# fused map F
grid_map(FMX, MID - FMS / 2, FMS, n=5, seed=2)
arrow([(CCX + CCW, MID), (FMX, MID)])
text(FMX + FMS / 2, MID + FMS / 2 + 34, v("F"), F_MATH, math=True)

# transformer decoder
box(TDX, VY, TDW, VH, "Transformer decoder", TRUNK, sub="3 layers, cross-attn to words", rot=True, fs=23, sfs=15)
arrow([(FMX + FMS, MID), (TDX, MID)])
WY = TROW + 64
arrow([(VX + VW, WY), (TDX + TDW / 2, WY), (TDX + TDW / 2, VY + VH)])
TKX = TDX + TDW / 2 - 190
tokens(TKX, WY - 78, 5, cell=22, gap=7)
text(TKX - 22, WY - 67, v("w"), F_MATH, anchor="end", math=True)
text(TKX + 70, WY - 30, "word tokens", F_SMALL, fill=MUTED)

# decoded map F_q
QY = MID - QS / 2
grid_map(QX, QY, QS, n=6, seed=1)
arrow([(TDX + TDW, MID), (QX, MID)])
text(QX + QS / 2, QY + QS + 34, "decoded map", F_ANN)
text(QX + QS / 2, QY + QS + 70, f'{v("F", "q")}&#8201; 512 × 32 × 32', F_SMALL, fill=MUTED)
...
text(HX + HS / 2, R1 + HS / 2 + 28, f'mask {sc("M")}', F_SMALL)
text(HX + HS + HG + HS / 2, R1 + HS / 2 + 28, "point", F_SMALL)
text(HX + HS + HG + HS / 2, R1 + HS / 2 + 58, "heatmap", F_SMALL)
arrow([(UX, R1), (BUS, R1), (BUS, ADY + 46), (ADX, ADY + 46)])
text(UX + 14, R1 + 30, "soft-argmax", F_SMALL, anchor="start", fill=MUTED)
text(UX + 190, R1 - 30, v("u", "p"), F_MATH, math=True)

# --- row 2: dense voting head -> per-pixel fields -> part-weighted mean
box(BX, R2 - BH / 2, BW, BH, "Dense voting\nhead", VOTE, sub="one vote per pixel", fs=24, sfs=15)
...
text((FLX + FEND) / 2, R2 + VS / 2 + 30, "per-pixel votes", F_SMALL)
text((FLX + FEND) / 2, R2 + VS / 2 + 66, f'{v("a", "i")}, {v("d", "i")}, {sc("c", "i")}, {v("o", "i")}', F_MATH - 3, math=True)
text((FLX + FEND) / 2, R2 + VS / 2 + 100, "axis, direction, type, hinge offset", F_SMALL - 2, fill=MUTED)
box(PMX, R2 - BH / 2, PMW, BH, "part-\nweighted\nmean", VOTE, fs=21, r=10)
arrow([(FEND, R2), (PMX, R2)])
# weights = the mask
WYY = R2 - BH / 2 - 66
arrow([(HX + HS / 2, R1 + HS / 2 + 50), (HX + HS / 2, WYY), (PMX + PMW / 2, WYY), (PMX + PMW / 2, R2 - BH / 2)], dashed=True, color=DIM)
text(PMX + PMW / 2 - 16, WYY + 27, f'weights {sc("w", "i")} = {sc("M")}', F_SMALL, anchor="end", fill=MUTED)
arrow([(PMX + PMW, R2), (ADX, R2)])
text((PMX + PMW + ADX) / 2, R2 - 34, f'{v("n")}, {v("d")}, {sc("c")}, {v("&#293;")}', F_MATH - 2, math=True)

# --- row 3: scalar MLPs
box(BX, R3 - BH / 2, BW, BH, "Part-pooled\nMLPs", SCAL, sub="depths and sweep", fs=24, sfs=15)
arrow([(QX + QS / 2, QY + QS + 96), (QX + QS / 2, R3), (BX, R3)])
arrow([(BX + BW, R3), (BUS, R3), (BUS, ADY + ADH - 46), (ADX, ADY + ADH - 46)])
text(PMX + PMW / 2, R3 - 34, f'{sc("z", "p")}, {sc("z", "q")}, {sc("L")}', F_MATH, math=True)

# ================================================================ decoder group
box(ADX, ADY, ADW, ADH, "Analytic\ndecoder", GEOM, sub="lift with K,\nrender the trajectory", sfs=15)
text(ADX + ADW / 2, ADY - 46, f'intrinsics {v("K")}', F_SMALL, fill=MUTED)
arrow([(ADX + ADW / 2, ADY - 26), (ADX + ADW / 2, ADY)], color=DIM)
...
text(OX + OS / 2, OY - 128, "mask, point, type,", F_SMALL - 2, fill=MUTED)
text(OX + OS / 2, OY - 100, "axis, hinge, sweep,", F_SMALL - 2, fill=MUTED)
text(OX + OS / 2, OY - 72, "trajectory", F_SMALL - 2, fill=MUTED)
text(OX + OS / 2, OY - 34, f'{sc("M")}, {v("p")}, {sc("c")}, {v("n")}, {v("q")}, {sc("L")}, {sc("γ")}(s)', F_MATH - 2, math=True)

# supervision of the decoded trajectory
box(S1X, SY - SH / 2, SW, SH, "2D projection loss", "white", sub="hand tracks,\nhuman video", fs=18, sfs=15, stroke=DIM)
box(S2X, SY - SH / 2, SW, SH, "3D closed-form loss", "white", sub="full-sweep arcs,\nSceneFun3D", fs=18, sfs=15, stroke=DIM)
arrow([(S1X + SW / 2, SY - SH / 2), (S1X + SW / 2, ADY + ADH)], dashed=True, color=DIM)
arrow([(S2X + SW / 2, SY - SH / 2), (S2X + SW / 2, OY + OS)], dashed=True, color=DIM)
text((S1X + S2X + SW) / 2, SY + SH / 2 + 36, "supervision of the decoded trajectory", F_SMALL, fill=MUTED)
