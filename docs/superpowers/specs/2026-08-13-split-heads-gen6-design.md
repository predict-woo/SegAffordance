...
| Mask / point / coords | unchanged | unchanged |
...
relative-MSE, DiceBCE mask / point-map BCE / coord L1 and their 0.5
weights.
...
- `point_source: "element"` — interaction point is the graspable
  element centroid, not the border-clamping projected hinge.
...
- GT-type input hint (`use_motion_type_input: false`) — twist-era
  crutch; the type head makes it redundant. `vae_condition` returns
  to the classical layout (no embedding concat).
...
- Eval additions: `origin_line_err_m` (the loss quantity, unsquared)
  and `radius_err` = | dist(element, pred axis line) - GT radius |
  on revolute rows. Axis direction error/type accuracy come from
  their heads directly; `twist_*` metrics disappear.

## Loss total

```
L = 0.5*L_mask + 0.5*L_point_map + 0.5*L_coord
...
in m^2 like the geometric terms). New `LossParams` fields:
`origin_weight` (0.5), plus `ModelParams.use_origin_head` (bool,
default false). The origin does NOT feed the cross-GT geometric loss —
...
- Unit: origin loss gauge invariance (translate GT origin along the
  ...
  (antiparallel prediction scores ~2, not 0); OriginMLP shape.
