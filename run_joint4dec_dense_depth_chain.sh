...
ulimit -n 65536
E=20260916_joint4_decoder_dense_depth; CFG=config/joint4_decoder_dense_depth.yaml
...
sed 's#/workspace/datasets/\(hoi4d_processed_2d_v2\|epic_processed_2d\|arctic_processed_2d\|sf3d_processed_v3\|sf3d_frames_512.lmdb\)#/dev/shm/\1#g' $CFG > /dev/shm/joint4dec_dense_depth_local.yaml
echo "staged paths: $(grep -c '/dev/shm/' /dev/shm/joint4dec_dense_depth_local.yaml)"; df -h /dev/shm | tail -1
mkdir -p experiments/$E/logs experiments/$E/checkpoints
echo "=== START $E $(date)"
HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_multi_better.py fit --config /dev/shm/joint4dec_dense_depth_local.yaml > experiments/$E/logs/train.log 2>&1
...
T() { logn=$1; shift; HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_SF3D_better.py test --config config/sf3d_test_decoder_rgb_scalefree.yaml --ckpt_path $ck --trainer.logger=false --model.model_params.articulation_readout dense --model.model_params.dense_hidden 256 --model.model_params.use_depth true --data.load_depth true --model.model_params.compile_model false --data.lmdb_path /dev/shm/data.lmdb --data.frame_cache_path /dev/shm/frames.lmdb --data.num_workers_val 8 "$@" > experiments/$E/logs/$logn 2>&1; echo "=== TESTDONE $logn exit=$?"; }
T test_sf3d.log
T test_sf3d_writerlen.log --model.model_params.trajectory_decoder_length writer
DEC="--model.model_params.use_depth true --model.model_params.articulation_readout dense --model.model_params.dense_hidden 256 --model.model_params.use_trajectory_head false --model.model_params.trajectory_decoder analytic --model.model_params.trajectory_dct_coeffs 0"
for c in hoi4d_v2 epic_v1 arctic_v1; do
  HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_SF3D_better.py test --config config/${c}_rgb_scalefree.yaml --ckpt_path $ck --trainer.logger=false $DEC --data.load_depth true --model.model_params.compile_model false --data.num_workers_val 8 > experiments/$E/logs/test_${c}.log 2>&1
