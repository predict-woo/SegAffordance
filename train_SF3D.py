...
class SF3DTrainingModule(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        point_gt_norm_sample = point_gt_norm[
            0:1
        ]  # (1, 2) keep batch dim for make_gaussian_map

        mask_pred_sigmoid_sample = torch.sigmoid(
            mask_pred_logits[0]
        )  # (1, H_map, W_map)
        point_pred_sigmoid_sample = torch.sigmoid(
            point_pred_logits[0]
        )  # (1, H_map, W_map)
        ...
        images_to_log = {
            ...
            "val_sample/gt_mask": mask_gt_sample.squeeze(
                0
            ),  # Remove channel dim if 1 for grayscale
...
def main():
    ...
    strategy_val = "auto"  # Default strategy
    ...
    if cfg.gpus > 0:
        ...
        if cfg.gpus == 1:
            ...
            strategy_val = "ddp_find_unused_parameters_true"  # Use DDP with unused parameter detection
    else:
        accelerator_type = "cpu"
        devices_val = 1
