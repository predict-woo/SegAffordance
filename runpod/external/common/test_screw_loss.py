...
from screw_loss import analytic_screw_trajectory, closed_form_screw_loss, screw_terms, axis_metrics
...
def test_screw_terms_wrapper_matches_core_and_masks_small_levers():
    mt, ax_p, q_p, p0_p, ax_gt, q_gt, p0_gt = rand_case(10, rot=True)
    t = screw_terms(mt, ax_p, q_p, p0_p, ax_gt, q_gt, p0_gt, min_radius=0.05)
    pos, der = closed_form_screw_loss(mt, ax_p, ax_p, q_p, p0_p, ax_gt, q_gt, p0_gt)
    assert t["pos"][0].item() == pytest.approx(pos[0].item())
    assert t["der"][0].item() == pytest.approx(der[0].item())
    assert bool(t["lever_ok"][0])
    # GT point on the axis -> lever ~0 -> masked out
    t2 = screw_terms(mt, ax_p, q_p, p0_p, ax_gt, q_gt, q_gt + 0.01 * ax_gt, min_radius=0.05)
    assert not bool(t2["lever_ok"][0])


def test_axis_metrics_sign_and_line_distance():
    n = np.array([0.0, 0.0, 1.0]); o = np.array([0.0, 0.0, 0.0])
    m = axis_metrics(-n, n, o + np.array([0.3, 0.0, 5.0]), o)
    assert m["flip"] == 1.0
    assert m["angle_signed_deg"] == pytest.approx(180.0)
    assert m["angle_unsigned_deg"] == pytest.approx(0.0, abs=1e-3)
    assert m["origin_line_dist"] == pytest.approx(0.3)   # slide along axis ignored
