...
for p in $(pgrep -f 'epic_axis_annotator.p[y]'); do kill "$p"; done   # the script's own command line does not contain the pattern
...
nohup /opt/venv/bin/python tools/epic_axis_annotator.py serve \
  ...
  --preds /workspace/SegAffordance/viz/20260916_epic_test_dump/preds.jsonl > /workspace/epic_annot.log 2>&1 < /dev/null &
sleep 3
