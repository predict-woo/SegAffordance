...
# USDNet resumes from <save_dir>/last-epoch.ckpt whenever save_dir exists and aborts if the
# file is missing: start fresh runs from an empty dir, let real reruns resume.
if [ "${SMOKE:-0}" = "1" ]; then
  rm -rf $RUNS
...
if [ ! -f $RUNS/.train_done ]; then
  [ -f $RUNS/last-epoch.ckpt ] || rm -rf $RUNS
  python main_instance_segmentation_articulation.py $COMMON general.experiment_name=$NAME general.save_dir=$RUNS data.train_mode=train \
...
cd /workspace/SegAffordance
python tools/baselines_sf3d/run.py tools/baselines_sf3d/usdnet_preds_to_jsonl.py --preds $RUNS/test/debug/val_preds/preds.pkl --usd-dir $D --out $RUNS/preds.jsonl
