...
[ -f /root/.done_pip_a3vlm ] || { pip install -q fairscale sentencepiece "transformers==4.34.1" "open_clip_torch==2.23.0" "timm==0.9.12" fire packaging pyyaml Ninja einops regex h5py pandas pyarrow tensorboard "gradio==3.50.2" "httpx[socks]" tqdm "huggingface_hub[cli]" pycocotools opencv-python-headless lmdb && touch /root/.done_pip_a3vlm; }
...
cd $R/A3VLM/model/accessory && python -c "import sys; sys.path.insert(0, '..'); from accessory.model.LLM import llama_ens5; from accessory.model.meta import MetaModel; print('a3vlm import ok')"
echo "== a3vlm env done"
