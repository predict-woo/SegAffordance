#!/bin/bash
# Pod mdec (2026-09-12 night): multi3 DECODER 2D arm (12 ep) -> union test + per-source tests -> SF3D DCT
# post-training from its best ckpt (sweep_queue stages SF3D) -> tests pred_z_p / gt_z0.
# Markers: MULTI3DEC_TRAIN_DONE, MULTI3DEC_TEST_DONE, POST_START, SF3D_DEC_DONE, CHAIN_DONE.
...
ulimit -n 65536
E=20260912_multi3_decoder_rgb_scalefree; CFG=config/multi3_decoder_rgb_scalefree.yaml
...
sed 's#/workspace/datasets/\(hoi4d_processed_2d_v2\|epic_processed_2d\|arctic_processed_2d\)#/dev/shm/\1#g' $CFG > /dev/shm/multi3dec_local.yaml
echo "staged paths: $(grep -c '/dev/shm/' /dev/shm/multi3dec_local.yaml)"
mkdir -p experiments/$E/logs experiments/$E/checkpoints
echo "=== START $E $(date)"
HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_multi_better.py fit --config /dev/shm/multi3dec_local.yaml > experiments/$E/logs/train.log 2>&1
...
echo "MULTI3DEC_TRAIN_DONE $(date)"
ck=$(ls experiments/$E/checkpoints/best-*.ckpt | head -1); [ -n "$ck" ] || { echo "NO_CKPT"; exit 1; }
HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_multi_better.py test --config /dev/shm/multi3dec_local.yaml --ckpt_path $ck --trainer.logger=false --data.num_workers_val 8 > experiments/$E/logs/test.log 2>&1
echo "=== TESTDONE union exit=$?"
for c in hoi4d_v2 epic_v1 arctic_v1; do
  HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_SF3D_better.py test --config config/${c}_rgb_scalefree.yaml --ckpt_path $ck --trainer.logger=false --model.model_params.use_trajectory_head false --model.model_params.trajectory_decoder analytic --model.model_params.trajectory_dct_coeffs 0 --model.model_params.compile_model false --data.num_workers_val 8 > experiments/$E/logs/test_${c}.log 2>&1
  echo "=== TESTDONE $c exit=$?"
done
echo "MULTI3DEC_TEST_DONE $(date)"
F=20260912_sf3d_decoder_l2anchor_ft_multi3dec; FCFG=config/sf3d_train_runpod_decoder_l2anchor_ft_multi3dec.yaml
...
rm -rf /dev/shm/hoi4d_processed_2d_v2 /dev/shm/epic_processed_2d /dev/shm/arctic_processed_2d /dev/shm/multi3dec_local.yaml
...
echo "SF3D_DEC_DONE $(date)"; echo "CHAIN_DONE $(date)"
