...
def compute_trajectory_3d_camera_coords(
    ...
    * Geometric consistency loss "pred vector <-> gt traj"
      ...
      the GT contract) the GT must satisfy exactly:
        ...
        - rot (:796-825, circle loss): (a) plane term ((Q_i - C) . n)^2 = 0
          -> every absolute point lies in the plane through the motion
    ...
    * Degenerate inputs (zero direction, no usable radius) fall back to a
    ...
    radius = 0.0
    e1 = None  # in-plane unit vector from origin towards the arc start
        ...
        for item_info in visible_items_info:
            ...
            if item_motion:
                ...
                if (
                    ...
                    visible_pts_world = laser_scan_points_full[
                        annotation.get("indices")
                    ][item_info["visible_mask_indices"]]
                    visible_pts_cam = np.zeros((0, 3), dtype=np.float64)
                    if visible_pts_world.size > 0:
                        pts_homo = np.hstack(
                            [
                                visible_pts_world,
                                np.ones((len(visible_pts_world), 1)),
                            ]
                        )
                        pts_cam_homo = (world_to_camera_pose @ pts_homo.T).T
                        visible_pts_cam = pts_cam_homo[:, :3] / pts_cam_homo[:, 3:4]
