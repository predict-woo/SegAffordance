...
| `poses.json` | Named arm poses. `home` (hand at x 0.70, z 0.30, pitched 15° down, = the supervisor's `half-down`) is the camera pose the pipeline starts and ends at. The arm jitters slightly there: the hand sits at the front face of a registered payload keep-out box (a 1.4 × 1.0 × 0.4 m "Custom" payload volume over the deck) and the arm's payload-collision avoidance fights the pose controller. x 0.85 was tried and is quiet, but from there the arm swings into the opened door when it returns home, so 0.70 stays. |
...
The camera should be at the "home" pose (`spotctl go home`: hand at x 0.70, z 0.30, pitched 15° down) for a
