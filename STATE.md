...
`frames.lmdb` — **359 records** (rebuilt ~04:10 with trajectories CUT at the narrated span end + 4 frames — the collaborator's tracks run 90 frames past the action; first build had 326 with wandering tails), SF3D reader format (same as HOI4D v2:
...
> 300 px (-9), < 5 points (-4). Per noun: drawer 170, fridge 73, cupboard
58, oven 24, dishwasher 9, others 25. Review sheets
`viz/20260908_epic_v1_random20` (current build: masks on the moving part
19/20, short open/close strokes) and `..._lmdb_sample` (pre-cut build). Reader smoke test passed
