...
class MotionVisualizer(Visualizer):
    def draw_gt_instance(
        self, anno, part_id_json, is_real=False, intrinsic_matrix=None, line_length=1.0
    ):
        ...
        origin_2d = [
            camera_to_image(origin, is_real, intrinsic_matrix) for origin in origins_4d
        ]
        ...
        circle_p_2d = np.asarray(
            [camera_to_image(p, is_real, intrinsic_matrix) for p in circle_p]
        )
    ...
    def draw_gt_instance_clean(
        ...
        self.overlay_instances(
            ...
            # boxes=boxes,
            masks=masks,
            assigned_colors=[_COLORS_LEVEL[1]],
            alpha=0.0,
        )
    ...
    def draw_pred_instance(
        ...
        gt_circle_p_2d = np.asarray(
            [camera_to_image(p, is_real, intrinsic_matrix) for p in gt_circle_p]
        )
        self.draw_line(
        ...
        pred_circle_p_2d = np.asarray(
            [camera_to_image(p, is_real, intrinsic_matrix) for p in pred_circle_p]
        )
    ...
    def draw_opd_predictions(self, predictions, intrinsic_matrix, line_length=0.1):
        """
        Draws predictions, including masks, labels, and motion vectors.

        Args:
            predictions (Instances): detectron2 Instances object containing the predictions.
            intrinsic_matrix (np.ndarray): 3x3 camera intrinsic matrix.
            line_length (float): The length of the motion vector to draw in meters.

        Returns:
            output (VisImage): image with visualizations.
        """
        # First, draw all the masks and boxes using the standard method
        # Create labels with motion type
        scores = predictions.scores if predictions.has("scores") else None
        classes = predictions.pred_classes if predictions.has("pred_classes") else None

        labels = None
        if classes is not None and self.metadata.get("thing_classes", None):
            class_names = self.metadata.get("thing_classes")
            motion_types = [MOTION_TYPE[t.item()] for t in predictions.mtype]
            labels = [f"{class_names[c]}_{mt}" for c, mt in zip(classes, motion_types)]
        ...
        # Now, draw motion vectors on top if intrinsics are available
        if intrinsic_matrix is None:
            return self.output

        for i in range(len(predictions)):
            pred_morigin = predictions.morigin[i].numpy()
            pred_maxis = predictions.maxis[i].numpy()
            pred_mtype = predictions.mtype[i].item()

            # Project 3D origin to 2D image plane. is_real=True uses the provided intrinsic_matrix.
            origin_2d = camera_to_image(
                pred_morigin, is_real=True, intrinsic_matrix=intrinsic_matrix
            )

            # Calculate 3D end point of the normalized axis vector
            norm_axis = pred_maxis / (np.linalg.norm(pred_maxis) + 1e-6)
            axis_endpoint_3d = pred_morigin + norm_axis * line_length
            endpoint_2d = camera_to_image(
                axis_endpoint_3d, is_real=True, intrinsic_matrix=intrinsic_matrix
            )

            # Define color for the arrow
            arrow_color = np.array([0, 255, 0]) / 255  # Green

            # Draw the main line of the arrow
            self.draw_line(
                [origin_2d[0], endpoint_2d[0]],
                [origin_2d[1], endpoint_2d[1]],
                color=arrow_color,
                linewidth=2,
            )

            # Create and draw the arrowhead
            vec_x = endpoint_2d[0] - origin_2d[0]
            vec_y = endpoint_2d[1] - origin_2d[1]
            arrow_wing1 = rotatePoint(vec_x, vec_y, 150, 0.25)
            arrow_wing2 = rotatePoint(vec_x, vec_y, -150, 0.25)
            self.draw_line(
                [endpoint_2d[0], endpoint_2d[0] - arrow_wing1[0]],
                [endpoint_2d[1], endpoint_2d[1] - arrow_wing1[1]],
                color=arrow_color,
                linewidth=2,
            )
            self.draw_line(
                [endpoint_2d[0], endpoint_2d[0] - arrow_wing2[0]],
                [endpoint_2d[1], endpoint_2d[1] - arrow_wing2[1]],
                color=arrow_color,
                linewidth=2,
            )

            # Draw a circle at the origin, with color depending on motion type
            if pred_mtype == 0:  # Rotation
                origin_color = np.array([255, 0, 0]) / 255  # Red
            else:  # Translation
                origin_color = arrow_color  # Green

            self.draw_circle(origin_2d, color=origin_color, radius=5)
