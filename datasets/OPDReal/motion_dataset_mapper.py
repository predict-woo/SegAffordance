    ...
    def __call__(self, dataset_dict):
        ...
        if self.image_format == "depth":
            ...
            image = self.depth_images[
                self.depth_filenames_map[dataset_dict["depth_file_name"]]
            ]
...
def bm_annotations_to_instances(
    ...
    if extrinsic_matrix is not None:
        if network_type != "BMOC_V1":
            ...
            extrinsic_matrix = torch.tensor(
                np.array([object_poses[object_key] for object_key in object_keys]),
                dtype=torch.float32,
            )
        ...
        if len(annos) == 0:
            ...
            transformation = torch.transpose(
                extrinsic_matrix.reshape(extrinsic_matrix.size(0), 4, 4), 1, 2
            )

            gt_translations = extrinsic_matrix[:, 12:15]
            gt_quaternions = matrix_to_quaternion(
                torch.transpose(
                    torch.cat(
                        [
            ...
            target.gt_extrinsic_quaternion = torch.cat(
                (gt_quaternions, gt_translations), 1
            )

            gt_6d_rotations = matrix_to_rotation_6d(
                torch.transpose(
                    torch.cat(
                        [
                            extrinsic_matrix[:, 0:3],
    ...
    if transformation is not None:
        temp_origins_cam = torch.tensor(
            np.array([origin + [1] for origin in origins_cam]), dtype=torch.float32
        )
    ...
    if transformation is not None:
        ...
        temp_axes_end_cam = torch.tensor(
            np.array([list(axis) + [1] for axis in axes_end_cam]), dtype=torch.float32
        )
