        ...
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    model_name = self.cocoGt.loadImgs(int(gt[m]["image_id"]))[0][
                        "file_name"
                    ].split("-")[0]
                    diagonal_length = self.diagonal_length[model_name]

                    if self.motionstate:
                        image_name = self.cocoGt.loadImgs(int(gt[m]["image_id"]))[0][
                            "file_name"
                        ].split(".")[0]
                    ...
                    if self.motionnet_type == "BMOC":
                        ...
                        dtOriginWorld[tind, dind] = (
                            np.linalg.norm(np.cross(p_world, gt_axis_world))
                            / np.linalg.norm(gt_axis_world)
                            / diagonal_length
                        )
                    ...
                    dtOrigin[tind, dind] = (
                        np.linalg.norm(np.cross(p, gt_axis))
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    ntrans = np.sum(
                        (gtType == MOTION_TYPE["translation"]) & (gtIg == 0)
                    )
                    if npig == 0:
                        continue
                    # MotionNet
                    ...
                    dtMotionThres_tps = np.logical_and(
                        np.logical_and.reduce(
                            (dtm, dtType, dtAxisThres, processOriginThres)
                        ),
                        np.logical_not(dtIg),
                    )
                    dtMotionThres_fps = np.logical_and(
                        np.logical_or.reduce(
                            (
                                np.logical_not(dtm),
                                np.logical_not(dtType),
                                np.logical_not(dtAxisThres),
                                np.logical_not(processOriginThres),
                            )
                        ),
                        np.logical_not(dtIg),
                    )
                    ...
                    dtTypeAxisThres_tps = np.logical_and(
                        np.logical_and.reduce((dtm, dtType, dtAxisThres)),
                        np.logical_not(dtIg),
                    )
                    ...
                    dtTypeAxisThres_tp_sum = np.cumsum(
                        dtTypeAxisThres_tps, axis=1
                    ).astype(dtype=np.float)
                    dtTypeAxisThres_fp_sum = np.cumsum(
                        dtTypeAxisThres_fps, axis=1
                    ).astype(dtype=np.float)
                    ...
                    if self.motionstate:
                        ...
                        dtState_tp_sum = np.cumsum(dtState_tps, axis=1).astype(
                            dtype=np.float
                        )
                        dtState_fp_sum = np.cumsum(dtState_fps, axis=1).astype(
                            dtype=np.float
                        )
                    ...
                    if (rot_index.shape[0] + trans_index.shape[0]) != 0:
                        nIns_total_pred[:, k, a, m] = (
                            rot_index.shape[0] + trans_index.shape[0]
                        )
                        matched_number = nIns_error_axis[:, k, a, m]
                        matched_number[np.where(matched_number == -1)] = 0
                        total_axis_scores[:, k, a, m] = (
                            dtAxis_sum[:, -1] * matched_number
                            + 90.0 * (nIns_total_pred[:, k, a, m] - matched_number)
                        ) / nIns_total_pred[:, k, a, m]
                        if rot_index.shape[0] != 0:
                            ...
                            total_origin_scores[:, k, a, m] = (
                                dtOrigin_sum[:, -1] * matched_rot_number
                                + 1.0
                                * (nIns_total_pred_rot[:, k, a, m] - matched_rot_number)
                            ) / nIns_total_pred_rot[:, k, a, m]
                            total_axis_rot_scores[:, k, a, m] = (
                                dtAxis_rot_sum[:, -1] * matched_rot_number
                                + 90.0
                                * (nIns_total_pred_rot[:, k, a, m] - matched_rot_number)
                            ) / nIns_total_pred_rot[:, k, a, m]
                        if trans_index.shape[0] != 0:
                            ...
                            matched_trans_number[
                                np.where(matched_trans_number == -1)
                            ] = 0
                            total_axis_trans_scores[:, k, a, m] = (
                                dtAxis_trans_sum[:, -1] * matched_trans_number
                                + 90.0
                                * (
                                    nIns_total_pred_trans[:, k, a, m]
                                    - matched_trans_number
                                )
                            ) / nIns_total_pred_trans[:, k, a, m]
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                        ...
                        if self.motionstate:
                            ...
                            dtState_pr = dtState_tp / (
                                dtState_tp + dtState_fp + np.spacing(1)
                            )
                        ...
                        typeaxis_inds = np.searchsorted(
                            typeaxis_rc, p.recThrs, side="left"
                        )
                        try:
                        ...
                        if self.motionstate:
                            state_inds = np.searchsorted(
                                state_rc, p.recThrs, side="left"
                            )
                            try:
                        ...
                        try:
                            ...
                            if nrot != 0:
                                nIns_mAP_rot[t, k, a, m] = (
                                    dtAxisThres_rot_tp[-1] + dtAxisThres_rot_fp[-1]
                                )
                            else:
                                nIns_mAP_rot[t, k, a, m] = -1
                            if ntrans != 0:
                                nIns_mAP_trans[t, k, a, m] = (
                                    dtAxisThres_trans_tp[-1] + dtAxisThres_trans_fp[-1]
                                )
                            else:
        ...
        def _summarizeDets():
            ...
            if self.PART_CAT == False:
                if self.MACRO_TYPE == False:
                    ...
                    _summarize(31)
                    stats[3] = _summarize(
                        31, iouThr=0.5, maxDets=self.params.maxDets[2]
                    )
                ...
                for cat in range(3):
                    if self.MACRO_TYPE == False:
                        _summarize(
                            1, iouThr=0.5, maxDets=self.params.maxDets[2], cat=cat
                        )
                        # MotionNet: motion type (different ious)
                        _summarize(
                            2, iouThr=0.5, maxDets=self.params.maxDets[2], cat=cat
                        )
                    # MotionNet: average motion threshold (different ious)
                    ...
                    if self.motionnet_type == "BMOC":
                        _summarize(
                            15, iouThr=0.5, maxDets=self.params.maxDets[2], cat=cat
                        )
                        _summarize(
                            16, iouThr=0.5, maxDets=self.params.maxDets[2], cat=cat
                        )
