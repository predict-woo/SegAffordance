#!/bin/bash
# H200 RESUME variant (2026-09-16): identical to run_joint4dec_dense_nomask_chain.sh except that the fit call resumes
# from experiments/$E/checkpoints/last.ckpt (copied from the EU-RO-1 pod). Data, caches, repo and checkpoint were copied
# to the AP-JP-1 volume in the SAME /workspace layout, so every path is unchanged.
# Pod jdec-dense (2026-09-13): joint decoder READOUT arm dense on the l2anchor recipe. Stage SF3D + the
...
ulimit -n 65536
E=20260916_joint4_decoder_dense_nomask; CFG=config/joint4_decoder_dense_nomask.yaml
...
sed 's#/workspace/datasets/\(hoi4d_processed_2d_v2\|epic_processed_2d\|arctic_processed_2d\|sf3d_processed_v3\|sf3d_frames_512.lmdb\)#/dev/shm/\1#g' $CFG > /dev/shm/joint4dec_dense_nomask_local.yaml
echo "staged paths: $(grep -c '/dev/shm/' /dev/shm/joint4dec_dense_nomask_local.yaml)"; df -h /dev/shm | tail -1
mkdir -p experiments/$E/logs experiments/$E/checkpoints
echo "=== START $E $(date)"
HF_HOME=/root/hfcache HF_HUB_OFFLINE=1 /opt/venv/bin/python train_multi_better.py fit --config /dev/shm/joint4dec_dense_nomask_local.yaml --ckpt_path experiments/$E/checkpoints/last.ckpt > experiments/$E/logs/train.log 2>&1
